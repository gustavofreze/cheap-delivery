...
variable "capacity_provider" {
  type        = string
  description = "Fargate capacity provider used by the service. Production runs exclusively on Fargate On-Demand."
  default     = "FARGATE"
...
variable "<provider>_api_base_url" {
  type        = string
  description = "Base URL of the <provider> API. Production targets the live API."
  default     = "https://<provider-host>/<api-version>"
}
...
variable "enable_public_dns_record" {
  type        = bool
  description = "Creates the public Route 53 alias record for the <service-name> hostname on the edge ALB. Disabled in production because the apex public zone lives in the master account."
  default     = false
}
