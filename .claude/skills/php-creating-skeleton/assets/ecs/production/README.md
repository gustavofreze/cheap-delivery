...
Provisions the service data of the <service-name> service in the production environment. The service-level plumbing
...
foundation is consumed through `terraform_remote_state` with key `<cloud-region>/environments/production.tfstate`, a
data source key that `-backend-config` cannot override, so a wrong value reads an empty state instead of failing the
plan. This stack keeps its own state at `services/<service-name>/<cloud-region>/production/<service-name>.tfstate`.
